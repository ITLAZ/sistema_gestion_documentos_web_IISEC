...
export class IdeasReflexionesService {
    constructor(
      ...
      async findByTitulo(titulo: string): Promise<IdeaReflexion[]> {
        return await this.IdeaReflexionModel.find({ titulo: { $regex: titulo, $options: 'i' } }).exec();
      }
    
      // Buscar IdeaReflexions por autor con aproximación
      async findByAutor(autor: string): Promise<IdeaReflexion[]> {
        return await this.IdeaReflexionModel.find({ autores: { $elemMatch: { $regex: autor, $options: 'i' } } }).exec();
      }  
      ...
      async update(id: string, IdeaReflexion: Partial<IdeaReflexion>): Promise<IdeaReflexion> {
        return this.IdeaReflexionModel.findOneAndUpdate({ id }, IdeaReflexion, { new: true }).exec();
      }
}
